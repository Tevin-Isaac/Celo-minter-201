...
describe("NFT Market", async function () {
  ...
  describe("Mint & List NFT", async function () {
    ...
    it(`List NFT in marketplace`, async function () {
      const listedNft = await marketplaceContract
        .connect(address1)
        .sellItem("www.myNFT.com", auctionPrice, nftContract.address, {
          value: mintingCost,
        });
      ...
      expect(event.events[4].args.token).to.equal(0);
    ...
    it("Should fail if listing price less then equal to zero ", async () => {
      await expect(
        marketplaceContract
          .connect(address1)
          .sellItem("www.myNFT.com", 0, nftContract.address, {
            value: mintingCost,
          })
    ...
    it("Should fail if minting cost is less then equal to zero ", async () => {
      ...
      ).to.be.revertedWith("You need to pay minting price");
    });
  });
  ...
  describe("Cancel NFT sell & Buy NFT", async function () {
    it(`Buy NFT`, async function () {
      ...
      var NFTSold = await marketplaceContract
        .connect(address2)
        .buyItem(0, { value: auctionPrice.toString() });
    ...
    it(`Cancel NFT`, async function () {
      ...
      const getNft = await marketplaceContract.Items(0);

      var sellCancel = await marketplaceContract
        .connect(address1)
        .cancelSell(0);
    ...
    it("Should fail if someone else cancel the sell ", async () => {
      ...
      await expect(
        marketplaceContract.connect(address2).cancelSell(0)
